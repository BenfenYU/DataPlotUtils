...
def plot_ecd_after_full_mud_properties():
    measure_ecd_path = r"C:\NotOneDrive\Data\algo_data_nas_copy\BDC 4-02\RawData\Channel_ECD_ESD.csv"
    ...
    measure_ecd = pd.read_csv(measure_ecd_path, skiprows=[1]).rename(
        columns={
            'Time': 'time_key',
            'ECD_ARC_RT': 'ecd_at_bit'
        }
    )
    measure_ecd['ecd_at_bit'] *= unit_convert('g/cm3', 'lb/gal')
    measure_ecd['time_key'] = pd.to_datetime(measure_ecd['time_key'], utc=False)

    mudin_310_algo_path = r"C:\NotOneDrive\Data\merged_input_output_channels\BDC-4-02_bha_run1_correct_mud_TransientHydraulics_20240805165848.csv"
    mudin_310_algo_ecd = pd.read_csv(mudin_310_algo_path, skiprows=[1])[['time key', 'ecd at bit']].rename(columns={'ecd at bit' : 'ecd_at_bit', 'time key': 'time_key'})
    mudin_310_algo_ecd['time_key'] = pd.to_datetime(mudin_310_algo_ecd['time_key'])
    mudin_310_algo_ecd['ecd_at_bit'] *= unit_convert('kg/m3', 'lb/gal')

    measure_ecd = measure_ecd[['time_key', 'ecd_at_bit']].dropna()
    measure_ecd = measure_ecd[measure_ecd['time_key'] < mudin_310_algo_ecd['time_key'].values[-1]]
    measure_ecd = measure_ecd[measure_ecd['time_key'] > mudin_310_algo_ecd['time_key'].values[0]]

    surface_cool_algo_path = r"C:\NotOneDrive\Data\merged_input_output_channels\BDC-4-02_bha_run1_correct_mud_TransientHydraulics_20240813175608_surface_cool.csv"
    surface_cool_algo_ecd = pd.read_csv(surface_cool_algo_path, skiprows=[1])[['time key', 'ecd at bit']].rename(columns={'ecd at bit' : 'ecd_at_bit', 'time key': 'time_key'})
    surface_cool_algo_ecd['time_key'] = pd.to_datetime(surface_cool_algo_ecd['time_key'])
    surface_cool_algo_ecd['ecd_at_bit'] *= unit_convert('kg/m3', 'lb/gal')
    ...
    fig.add_trace(
        go.Scattergl(
            ...
            line={'width': line_width},
            x=mudin_310_algo_ecd['time_key'], 
            y=mudin_310_algo_ecd['ecd_at_bit'], 
            name="PA(based on mud in temp) ecd at bit",
            yaxis='y'),
    )
    ...
    fig.add_trace(
        go.Scattergl(
            ...
            line={'width': line_width},
            x=surface_cool_algo_ecd['time_key'], 
            y=surface_cool_algo_ecd['ecd_at_bit'], 
            name="PA(based on surface cool temp) ecd at bit",
            yaxis='y'),
    )
    ...
    fig.update_layout(
        title = dict(text=fr'Validate PA TransientHydraulics', font = dict(size = 30)),
        yaxis=dict(title=dict(text = "Ecd (lb/gal)", font = dict(size = 20)),),
        xaxis=dict(title=dict(
            text = "Time",
            font = dict(size = 20)
